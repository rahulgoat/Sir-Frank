...
def main() -> None:
    ...
    # Get the primary assistant Frank
    ...
    if last_message.get("role") == "user":
        ...
        # Initialize and call David for detailed case research
        logger.info(f"---*--- Creating Case Research Agent (David) ---*---")
        case_research_agent = get_case_research_agent(llm_model=llm_model)
        case_research_agent.create_run()
        with st.chat_message("assistant"):
            resp_container = st.empty()
            response = ""
            for delta in case_research_agent.run(question):
                response += delta  # type: ignore
                resp_container.markdown(response)
            st.session_state["messages"].append({"role": "assistant", "content": response})
